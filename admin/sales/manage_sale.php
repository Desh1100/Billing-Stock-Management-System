...
<div class="card card-outline card-primary">
                ...
                <table class="table table-striped table-bordered" id="list">
                    ...
                    <thead>
                        <tr class="text-light bg-navy">
                            ...
                            <th class="text-center py-1 px-2"> Unit Cost</th>
                            <th class="text-center py-1 px-2"> Sub Total</th>
                        </tr>
                    </thead>
                    <tbody>
                    ...
                    <?php 
                        $total = 0;
                        if(isset($id)):
                        $qry = $conn->query("SELECT s.*,i.name,i.description, c.category_name 
                            FROM `stock_list` s 
                        ...
                        <tr>
                            ...
                            <td class="py-1 px-2 text-center category">
    ...
    <?php echo number_format($row['price']); ?>
                    ...
                    <tr>
                            ...
                            <th class="text-right py-1 px-2 sub-total"><?php echo number_format($total); ?></th>
                        </tr>
                        <tr>
                            <th class="text-right py-1 px-2" colspan="5">Discount <input style="width:40px !important" name="dispercent" class='' type="number" min="0" max="100" value="<?php echo isset($dispercent) ? $dispercent : 0 ?>">%
                                <input type="hidden" name="discount" value="<?php echo isset($discount) ? $discount : 0 ?>">
                            </th>
                            <th class="text-right py-1 px-2 discount"><?php echo number_format($discount); ?></th>
                        </tr>
                        <tr>
                            <th class="text-right py-1 px-2" colspan="5">Tax <input style="width:40px !important" name="taxpercent" class='' type="number" min="0" max="100" value="<?php echo isset($taxpercent) ? $taxpercent : 0 ?>">%
                                <input type="hidden" name="tax" value="<?php echo isset($tax) ? $tax : 0 ?>">
                            </th>
                            <th class="text-right py-1 px-2 tax"><?php echo number_format($tax); ?></th>
                        </tr>
                        <tr>
...
<script>
    ...
    $(function(){
        ...
        $('#add_to_list').click(function(){
            ...
            if(item == '' || qty == '' || unit == ''){
                alert_toast('Form item fields are required.', 'warning');
                return false;
            }

            if($('table#list tbody').find('tr[data-id="'+item+'"]').length > 0){
                alert_toast('Item is already on the list.', 'error');
                return false;
            }
        ...
        $('[name="dispercent"], [name="taxpercent"]').on('input', function(){
            calc();
        });
    ...
    function calc(){
        ...
        var discount_perc = parseFloat($('[name="dispercent"]').val()) || 0;
        var discount = (discount_perc / 100) * sub_total;

        var tax_perc = parseFloat($('[name="taxpercent"]').val()) || 0;
        var tax = (tax_perc / 100) * (sub_total - discount);

        var grand_total = sub_total - discount + tax;

        $('table#list tfoot .sub-total').text(parseFloat(sub_total).toLocaleString('en-US', {style: 'decimal', maximumFractionDigits: 2}));
        $('table#list tfoot .discount').text(parseFloat(discount).toLocaleString('en-US', {style: 'decimal', maximumFractionDigits: 2}));
        $('table#list tfoot .tax').text(parseFloat(tax).toLocaleString('en-US', {style: 'decimal', maximumFractionDigits: 2}));
        $('table#list tfoot .grand-total').text(parseFloat(grand_total).toLocaleString('en-US', {style: 'decimal', maximumFractionDigits: 2}));

        $('[name="discount"]').val(parseFloat(discount)); // Update discount amount input field
        $('[name="tax"]').val(parseFloat(tax)); // Update tax amount input field
