            ...
            <div class="container-fluid">
                ...
                <table class="table table-striped table-bordered" id="list">
                    ...
                    <tbody>
                        ...
                        if(isset($po_id)):
                        if(!isset($bo_id))
                        $qry = $conn->query("SELECT p.*,i.name,i.description, i.category_id, c.category_name FROM `po_items` p inner join item_list i on p.item_id = i.id inner join category_list c on i.category_id = c.id where p.po_id = '{$po_id}'");
                        else
                        $qry = $conn->query("SELECT b.*,i.name,i.description, i.category_id, c.category_name FROM `bo_items` b inner join item_list i on b.item_id = i.id inner join category_list c on i.category_id = c.id where b.bo_id = '{$bo_id}'");
                        ...
                        <tr>
                            ...
                            <td class="py-1 px-2 text-center qty">
                                <input type="number" name="qty[]" style="width:75px !important" value="<?php echo $row['qty']; ?>" max = "<?php echo $row['quantity']; ?>" min="0">
