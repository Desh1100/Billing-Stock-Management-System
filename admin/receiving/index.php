                    ...
                    <tbody>
                        ...
                        while ($row = $qry->fetch_assoc()):
                            ...
                            if (!empty($row['form_id'])) {
                                if ($row['from_order'] == 1) {
                                    $result = $conn->query("SELECT po_code FROM `purchase_order_list` WHERE id='{$row['form_id']}'");
                                    if ($result && $result->num_rows > 0) {
                                        $code = $result->fetch_assoc()['po_code'];
                                    }
                                } else {
                                    $result = $conn->query("SELECT bo_code FROM `back_order_list` WHERE id='{$row['form_id']}'");
                                    if ($result && $result->num_rows > 0) {
                                        $code = $result->fetch_assoc()['bo_code'];
                                    }
                                }
                            }
                            ?>
